...
```
#---------Gráficas para el articulo----------------




```{r,fig.height=5, fig.width=5}
T1 <- sort(prop.table(table(bf$zona)), decreasing = T)


text(x = barplot(T1, main="Zonas", 
      xlab="Zona", ylab="Frecuencia", 
      col = c("#BC8F8F","#CAFF70", "#A2B5CD"), ylim = c(0,1)),
     y = T1, labels = paste(round(T1*100, 2), "%"), pos = 3)


```
```{r, fig.height=4, fig.width=4}
#library(lessR)
T2 <- prop.table(table(bf$sexo_))

PieChart(T2, hole = 0, values = "%", data = T2,
         fill = c("#FFB5C5","#ffff81"), 
         main = "Proporción Sexo", values_color="black", cex = 0.9)

```

```{r, fig.height=5, fig.width=5}
T3 <- sort(prop.table(table(bf$tipo_ss_2)), decreasing = T)


text(x = barplot(T3, main="Tipo de Seguridad social", 
      xlab="Seguridad social", ylab="Frecuencia", 
      col = c("#8B4513", "#8B3E2F", "#8B6508"), ylim = c(0,1)),
     y = T3, labels = paste(round(T3*100, 2), "%"), pos = 3)

```

```{r, fig.height=5, fig.width=5}
T4 <-  prop.table(table(bf$pac_hos_))
names(T4) <- c("SI", "NO")
T4
text(x = barplot(T4, main="Frecuencia Paciente hospitalizado", 
      xlab="Paciente hospitalizado", ylab="Frecuencia", 
      col = c("#FA8072", "#87CEFA"), ylim = c(0,1)),
     y = T4, labels = paste(round(T4*100, 2), "%"), pos = 3)

?barplot()
```
```{r, fig.height=5, fig.width=5}
# Esquema de vacunación vs Paciente hospitalizado
t5 <- prop.table(table(bf$pac_hos_, bf$esq_vac))
colnames(t5) <- c("SI", "NO", "Desconocido")
rownames(t5) <- c("SI", "NO")
t5

text(x= barplot(t5, beside=TRUE, 
        main="Esquema de vacunación vs Paciente hospitalizado",
        xlab="Esquema completo", ylab="Frecuencia",
        col = c("#FA8072", "#87CEFA"),legend = rownames(t5),
        ylim = c(0,1), cex.main = 1),y= t5,
     labels = paste(round(t5*100, 2), "%"), pos = 3, cex = 0.8)

```
```{r, fig.height=5, fig.width=5}
# S.S vs Paciente hospitalizado
t6 <- prop.table(table(bf$pac_hos_, bf$tipo_ss_2))
rownames(t6) <- c("SI", "NO")
t6 <- t6[,c(1,3,2)]

t6

text(x= barplot(t6, beside=TRUE, 
        main="Seguridad Social vs Paciente hospitalizado",
        xlab="Tipo de Seguridad Social ", ylab="Frecuencia",
        col = c("#FA8072", "#87CEFA"),legend = rownames(t6),
        ylim = c(0,1), cex.main = 1),y= t6,
     labels = paste(round(t6*100, 2), "%"), pos = 3, cex = 0.8)
...
```{r}
T7 <- table(bf$pac_hos_, bf$sexo_)
T7

T8 <- table(bf$pac_hos_, bf$zona)
T8
